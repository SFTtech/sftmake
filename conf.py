...
class Var:
	...
	def eval(self, evalconf, depends = OrderedSet()):
		...
		if self.assscope == Var.SCOPE_CONF:
			am = ((assignment, conf) for conf in evalconf.parenthyperres() for assignment in self.assignments.get(conf, []))

		elif self.assscope == Var.SCOPE_GLOBAL:
			am = ((assignment, conf_default) for assignment in self.assignments)

		#iterate over all assignments that were generated by above parent resolvings
		for (assignment, conf) in am:
			#first, check the condition

			if not assignment.condtree.eval(evalconf, depends):
				continue

			vals = assignment.valtree.eval(evalconf, self.valtype, depends)
