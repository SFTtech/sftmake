...
class BuildTarget(Compilation):
	...
	def run(self):
		...
		if(self.needs_build):
			...
			if(self.prebuild):
				print("prebuild for " + t_name + " '" + self.prebuild + "'")
			...
			if(ret != 0):
				...
				if(self.postbuild):
					print("postbuild for " + t_name + " '" + self.postbuild + "'")
	...
	def run(self):
		...
		if(self.needs_build):
			...
			if(not self.target == None):
				#notify the target it needs to be built
				self.target.needs_build = True
