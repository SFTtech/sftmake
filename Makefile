...
$(TARGETFILE): parser
	...
	@for FILE in $(PARTSDIR)/*; do PART=$$(echo "parts/$$(readlink "$$FILE")"); echo -e "\n\n\n# BEGIN FILE $$PART\n\n\n" >> $(TARGETFILE) && cat $$PART >> $(TARGETFILE) && echo "Adding $$PART"; done
	@#add final lines
...
testparts:
	@for FILE in $(PARTSDIR)/*; do PART=$$(echo "parts/$$(readlink "$$FILE")"); echo ./$$PART; ${PYTHON} $$PART || exit 1; done

#this is the quiet version of testparts
testpartsq:
	@for FILE in $(PARTSDIR)/*; do PART=$$(echo "parts/$$(readlink "$$FILE")"); echo ./$$PART '1>/dev/null'; ${PYTHON} $$PART 1>/dev/null || exit 1; done

autoparsertest: parser
	PYTHONPATH=. ${PYTHON} ./tests/autoparsertest.py

#this is the quiet version of autoparsertest
autoparsertestq: parser
	PYTHONPATH=. ${PYTHON} ./tests/autoparsertest.py 1>/dev/null
...
runtests:
	...
	@for FILE in $(TESTSDIR)/*; do PART=$$(echo "parts/$$(readlink "$$FILE")"); echo ./$$PART; ${PYTHON} $$PART || exit 1; done
